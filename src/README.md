...
问答流程拆成三个页面：`qa.html` 负责写下问题，`qa-session.html` 负责多轮对话，`qa-results.html` 负责展示路径、完整 AI 回答与知乎公开来源。页面之间通过同一标签页的 `sessionStorage` 和 URL 中的 `session_id` 交接，因此刷新结果页仍可回到当前会话。毕业、就业、求职、读博等已有主题使用本地精选内容完成“问题—追问—路径—知乎公开来源”流程，其他问题进入实时后端链路。开发联调时可用 `qa.html?mode=live` 强制实时链路，或用 `qa.html?mode=curated` 强制精选链路。
